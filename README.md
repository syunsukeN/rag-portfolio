...
**現在のバージョン: v2.3.0**（出典検証機能追加）

## 🎯 このプロジェクトで示したいこと

| 観点 | 実装内容 |
|------|----------|
| AIを直接信用しない設計 | プロンプト制御 + コード側の出典検証（v2.3.0） |
...
```json
{
  ...
  "answer": "入社6ヶ月後に10日付与されます。(attendance.md - 有給休暇)",
  "clarification": null,
  ...
  "verification": {
    "citation_presence": {
      ...
      "mentioned_sources": ["attendance.md"],
      "unexpected_sources": [],
      "warning": null
    }
  },
...
「資料にないことは答えない」をプロンプト依存からコード側でも検証。

**検証内容:**

| 検証項目 | 説明 |
|----------|------|
| 出典表記チェック | 回答に`*.md`ファイル名が含まれるか |
| 出典捏造検出 | 回答に存在しないファイル名が含まれていないか |
| 段階判定 | strong（完全一致）/ weak（ベース名のみ）/ none |

**検証失敗時の挙動:**

| 状況 | confidence調整 | フロントエンド |
|------|---------------|---------------|
| 出典なし | 1段階ダウン | 警告バッジ表示 |
| 出典捏造検出 | lowに固定 | 警告バッジ表示 |
| 弱い一致のみ | 維持 | 弱い警告表示 |

**なぜこの機能が必要か:**

プロンプトで「資料にないことは答えない」と指示しても、LLMが指示を無視する可能性があります。コード側で回答を機械的にチェックすることで、プロンプト依存のリスクを軽減しています。
...
- **出典検証警告**: 回答の信頼性に問題がある場合に警告バッジを表示
